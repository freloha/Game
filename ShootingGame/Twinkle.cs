...
public class Twinkle : MonoBehaviour
{
    ...
    private IEnumerator FadeEffect(float start, float end)
    {
        ...
        while(percent < 1)
        {
            ...
            color.a = Mathf.Lerp(start, end, percent); // 선형 보간을 통한 서서히 값을 증가시키는 함수, color.a는 선명도에 대한 값
            // Mathf.Lerf(start, end, percent); == start와 end 사이의 값 중 percent 위치에 있는 값을 반환, start가 0이고 end가 100일 때, percent가 0.3이면 30을 반환
            spriteRenderer.color = color;

            yield return null; // yield return은 컬렉션 데이터를 하나씩 리턴하는데 사용, 일종의 브레이크 포인트 역할
            /*
             * yield return 1;
             * yield return 2;
